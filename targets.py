...
def calc_priority(targets, zcat, bright=None):
    """
    ...
    if np.all(unobs):
        ...
        spectype = np.repeat('UNOBS', len(targets))
    else:
        nmore = zcat['NUMOBS_MORE']
        spectype = zcat['SPECTYPE']
    ...
    if survey != 'cmx':
        if (desi_target in targets.dtype.names) and (not bright):
            ...
            for name in names:
                ii = (targets[desi_target] & desi_mask[name]) != 0
                jj = np.in1d(targets['TARGETID'], zcat['TARGETID'][zcat['SPECTYPE']=='GALAXY'])
                priority[ii & unobs] = np.maximum(priority[ii & unobs], desi_mask[name].priorities['UNOBS'])
                priority[ii & done & jj] = np.maximum(priority[ii & done & jj],  desi_mask[name].priorities['DONE'])
                priority[ii & zgood & jj] = np.maximum(priority[ii & zgood & jj], desi_mask[name].priorities['MORE_ZGOOD'])
                priority[ii & zwarn] = np.maximum(priority[ii & zwarn], desi_mask[name].priorities['MORE_ZWARN'])

            # QSO could be Lyman-alpha or Tracer.
            name = 'QSO'
            ii = (targets[desi_target] & desi_mask[name]) != 0
            jj = np.in1d(targets['TARGETID'], zcat['TARGETID'][zcat['SPECTYPE']=='QSO'])
            good_hiz = zgood & (zcat['Z'] >= 2.15) & (zcat['ZWARN'] == 0)
            priority[ii & unobs] = np.maximum(priority[ii & unobs], desi_mask[name].priorities['UNOBS'])
            priority[ii & done & jj] = np.maximum(priority[ii & done & jj], desi_mask[name].priorities['DONE'])
            priority[ii & good_hiz & jj] = np.maximum(priority[ii & good_hiz & jj], desi_mask[name].priorities['MORE_ZGOOD'])
            priority[ii & ~good_hiz & jj] = np.maximum(priority[ii & ~good_hiz & jj], desi_mask[name].priorities['DONE'])
            priority[ii & zwarn] = np.maximum(priority[ii & zwarn], desi_mask[name].priorities['MORE_ZWARN'])

        # BGS targets.
        if (bgs_target in targets.dtype.names) and (bright):
        ...
        if mws_target in targets.dtype.names:
            for name in mws_mask.names():
                ii = (targets[mws_target] & mws_mask[name]) != 0
                jj = np.in1d(targets['TARGETID'], zcat['TARGETID'][zcat['SPECTYPE']=='STAR'])
                priority[ii & unobs] = np.maximum(priority[ii & unobs], mws_mask[name].priorities['UNOBS'])
                priority[ii & done & jj] = np.maximum(priority[ii & done & jj],  mws_mask[name].priorities['DONE'])
                priority[ii & zgood & jj] = np.maximum(priority[ii & zgood & jj], mws_mask[name].priorities['MORE_ZGOOD'])
